...
- [ ] **Improved editing UI**: The UI for creating changelogs is *fine* as of right now, but can become pretty cluttered when a lot of text is entered (especially with MiniMessage). This makes mistakes way more likely (very much from my own experience), so a UI that allows the user to **preview** the changelog they're writing (including the built-in formatting for lines) and edit it on the go (e.g. through "add line" / "edit line" buttons with the lines rendered belwo) would be nice to have!
